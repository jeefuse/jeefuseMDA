...
public class EnumerationRender implements Render {
	...
	public EnumerationRender(String prefix, Class uml2Class, Property property, Property parent) {
		this.prefix = prefix;
		this.parent = parent;
	...
	protected void onchange(){
		tag.append(" onchange=\"valideteFile(this,'") ;
		tag.append(getFileAcceptType());
		tag.append("');\"");
	}

	protected String getFileAcceptType(){
		return Config.fileDefaultAcceptType;
	}
	
	protected void styleClass() {
		tag.append( "class=\"file");
		if (isRequired()) {
			tag.append(" required");
		}
		tag.append("\"");
