...
import net.jeefuse.mda.annotations.model.TreeListStyle;

public class TreeStereotypeHelper {
	...
	static public String getValue_treeListStyle(Class element) {
		String attribute=TreeAnnotations.treeListStyle.toString();
		String value=StereotypeUtil.getAppliedValueForString(element, stereotype, attribute);
		if(StringUtils.isBlank(value))
			return TreeListStyle.grid.toString();
		return value;
	}
	...
	static public Property getProperty_layerCode(Class element){
		Property property=null;
		String stereotypeValue=getValue_layerCode(element);
		if(StringUtils.isNotBlank(stereotypeValue)){
			property= UmlHelper.getPropertyElement(element,stereotypeValue );
		}
		//应用了stereotype:ListLabel
