...
public class EntityManagerJS
{
  protected static String nl;
  public static synchronized EntityManagerJS create(String lineSeparator)
  {
    nl = lineSeparator;
    EntityManagerJS result = new EntityManagerJS();
    nl = null;
    return result;
  }
  ...
  protected final String TEXT_35 = "',//获取每行唯一标识值的属性名,(前缀row+该属性值)组成每行DOM元素的唯一标识,通过grid.getIndexId()获取." + NL + "\t\tsortname: \"";
  protected final String TEXT_36 = "\",//默认排序属性名" + NL + "\t\tsortorder: \"";
  protected final String TEXT_37 = "\",//默认排序方向" + NL + "\t\tautoload: true,//是否初始时加载数据" + NL + "\t\tshowcheckbox: true,//是否显示checkbox" + NL + "\t\tusepager: true,//是否显示分页" + NL + "\t\trowbinddata: false,//是否每行绑定对象.设置为true,可通过$(rowDom).data('rowData')取得该行的对象" + NL + "        rowhandler: gridContextmenu,//右键菜单" + NL + "\t\tsearchparam:searchparamsCall,//搜索参数,数组或键值对" + NL + "\t\tonRowDblclick:onRowDblclick//左键双击执行" + NL + "    };" + NL + "    " + NL + "\t//format" + NL + "\t";
  protected final String TEXT_38 = NL + "\tfunction format";
  protected final String TEXT_39 = "(value, pid) {" + NL + "\t\treturn ";
  protected final String TEXT_40 = "[value];" + NL + "\t}";
  protected final String TEXT_41 = NL + NL + "    //search params" + NL + "    function searchparamsCall(){" + NL + "\t\tvar params=$(\"#firstSearchForm\").serializeArray();" + NL + "     \treturn params;" + NL + "\t}" + NL + "\t" + NL + "\t//grid" + NL + "    grid = $(\"#dataGrid\").flexigrid(option);" + NL + "    " + NL + "    //validate";
  ...
  protected final String TEXT_78 = NL + "\t//category tree " + NL + "    loadCategoryTree();";
  protected final String TEXT_79 = NL + "    //variate";
  protected final String TEXT_80 = NL + "    config.inputUrl=ctx+'";
  protected final String TEXT_81 = "';";
  protected final String TEXT_82 = "    " + NL + "    config.editUrl=ctx+'";
  protected final String TEXT_83 = "';";
  protected final String TEXT_84 = "    " + NL + "    config.deleteOutJsonUrl=ctx+'";
  protected final String TEXT_85 = "';";
  protected final String TEXT_86 = "   " + NL + "    config.deleteOutJsonAllUrl=ctx+'";
  protected final String TEXT_87 = "';";
  protected final String TEXT_88 = NL + "});" + NL + "var grid;" + NL + "var searchFormValidator;" + NL + "var config={" + NL + "\tbaseTitle:\"";
  protected final String TEXT_89 = "管理\"," + NL + "\tinputUrl:null," + NL + "\teditUrl:null," + NL + "\tdeleteOutJsonUrl:null," + NL + "\tdeleteOutJsonAllUrl:null" + NL + "};";
  protected final String TEXT_90 = "\t";
  protected final String TEXT_91 = NL + "/******grid context menu*******/" + NL + "function gridContextMenu(row) {" + NL + "\t$(row).contextmenu(menu);" + NL + "} " + NL + "function gridContextMenuClick(target,cmd) {" + NL + "\tvar id = $(target).attr(\"id\").substr(3);" + NL + "\tvar rowIdx = $(target).attr(\"seq\");" + NL + "\tswitch(cmd){";
  protected final String TEXT_92 = NL + "\t\tcase 'add':" + NL + "\t\t\tvar title=config.baseTitle+'->新增';" + NL + "\t\t\tnewWinOpen({title:title});" + NL + "\t\t\tbreak;";
  protected final String TEXT_93 = "\t\t\t" + NL + "\t\tcase 'edit':" + NL + "\t\t\tvar title = config.baseTitle+'->编辑 (序号:' + rowIdx + ')';" + NL + "\t\t\tvar url=goolov.addUrlParams(config.editUrl,\"id=\"+id);" + NL + "\t\t\teditWinOpen( {title:title,url:url});" + NL + "\t\t\tbreak;";
  protected final String TEXT_94 = " " + NL + "\t\tcase 'deleteCurrent':" + NL + "\t\t\tdeleteOutJson(new Array(id));" + NL + "\t\t\tbreak;" + NL + "\t\tcase 'deleteSels':" + NL + "\t\t\tdelData();" + NL + "\t\t\tbreak;";
  protected final String TEXT_95 = NL + "\t\tdefault:" + NL + "\t\t\tgrid.flexReload();" + NL + "\t}" + NL + "};" + NL + "/******category tree ***************/" + NL + "function loadCategoryTree(){" + NL + "\tvar url=ctx+'";
  protected final String TEXT_96 = "';" + NL + "    var options = { url:url,onnodeclick:treeNodeClick,rowhandler:treeContextMenu,showcheck: false,iconspath:ctx+\"/resources/style/default/tree/images/\"/*,theme: \"bbit-tree-lines\"*/ };" + NL + "    $('#firstTree').ajaxTree(options);" + NL + "}" + NL + "function treeNodeClick(item) {" + NL + "    if(!item.id||'root'==item.id){" + NL + "    \t$(\"#";
  protected final String TEXT_97 = "_";
  protected final String TEXT_98 = "\").val('');" + NL + "    }else{" + NL + "    \t$(\"#";
  protected final String TEXT_99 = "_";
  protected final String TEXT_100 = "\").val(item.id);" + NL + "    }" + NL + "\treloadGrid({newp:1});" + NL + "}";
  protected final String TEXT_101 = NL + "/******contenxtmenu*****/" + NL + "var imgPath=ctx+\"/resources/style/default/contextmenu/images/\";" + NL + "var menu = {" + NL + "\t\twidth : 150" + NL + "\t\t,items : [" + NL + "\t\t\t\t{text : \"刷新\",icon : imgPath+\"table_refresh.png\",alias : \"refresh\",action : contextMenuClick}" + NL + "\t\t\t\t,{type: \"splitLine\" }";
  protected final String TEXT_102 = NL + "\t\t\t\t,{text : \"新增\",icon :imgPath+\"view.png\",alias : \"add\",action :contextMenuClick}";
  protected final String TEXT_103 = NL + "\t\t\t\t,{text : \"编辑\",icon : imgPath+\"edit.png\",alias : \"edit\",action : contextMenuClick}";
  protected final String TEXT_104 = NL + "\t\t\t\t,{type: \"splitLine\" }" + NL + "\t\t\t\t,{text : \"删除该项记录\",icon : imgPath+\"rowdelete.png\",alias : \"deleteCurrent\",action : contextMenuClick}" + NL + "\t\t\t\t,{text : \"删除所选记录\",icon : imgPath+\"rowdelete.png\",alias : \"deleteSels\",action : contextMenuClick}";
  protected final String TEXT_105 = NL + "\t\t\t\t]" + NL + "\t};" + NL + "function contextMenuClick(target) {" + NL + "\tvar id = $(target).attr(\"id\").substr(3);" + NL + "\tvar rowIdx = $(target).attr(\"seq\");" + NL + "\tvar cmd = this.data.alias;" + NL + "\tswitch(cmd){";
  protected final String TEXT_106 = "\t" + NL + "\t\tcase 'add':" + NL + "\t\t\tvar title=config.baseTitle+'->新增';" + NL + "\t\t\tnewWinOpen({title:title});" + NL + "\t\t\tbreak;";
  protected final String TEXT_107 = NL + "\t\tcase 'edit':" + NL + "\t\t\tvar title = config.baseTitle+'->编辑 (序号:' + rowIdx + ')';" + NL + "\t\t\tvar url=goolov.addUrlParams(config.editUrl,\"id=\"+id);" + NL + "\t\t\teditWinOpen( {title:title,url:url});" + NL + "\t\t\tbreak;";
  protected final String TEXT_108 = NL + "\t\tcase 'deleteCurrent':" + NL + "\t\t\tdeleteOutJson(new Array(id));" + NL + "\t\tcase 'deleteSels':" + NL + "\t\t\tdelData();" + NL + "\t\t\tbreak;";
  protected final String TEXT_109 = NL + "\t\tcase 'refresh':" + NL + "\t\t\trefresh();" + NL + "\t\t\tbreak;" + NL + "\t}" + NL + "};" + NL + "function gridContextmenu(row) {" + NL + "\t$(row).contextmenu(menu);" + NL + "} ";
  protected final String TEXT_110 = "\t" + NL + "" + NL + "/******searchForm validate check******/" + NL + "function searchFormValidCheck(){" + NL + "\tsearchFormValidator.form();" + NL + "\tif(!searchFormValidator.valid()){" + NL + " \t\tvar invalideNum=searchFormValidator.numberOfInvalids();" + NL + "\t    if(invalideNum>0){" + NL + "\t    \tgoolov.growl.success('有<font color=red> '+invalideNum+'</font> 个查询选项未填写正确!');" + NL + "\t\t    return false;" + NL + "\t    }" + NL + "\t}" + NL + "\treturn true;" + NL + "}" + NL + "/******refresh grid *******/" + NL + "function reloadGrid(setting){" + NL + "\tif(searchFormValidCheck()){" + NL + "\t\tgrid.flexReload(setting?setting:{});" + NL + "\t}" + NL + "}" + NL + "/******refresh  *******/" + NL + "function refresh(){" + NL + "\treloadGrid();" + NL + "}" + NL + "/******reset clear *******/" + NL + "function clear(){" + NL + "\tdocument.getElementById('firstSearchForm').reset();" + NL + "\tsearchFormValidator.resetForm();" + NL + "\treloadGrid({newp:1});" + NL + "}" + NL + "/******search*******/" + NL + "function firstSearch(){" + NL + "\treloadGrid({newp:1});" + NL + "}";
  protected final String TEXT_111 = NL + "/******add data**********/" + NL + "var newWin;" + NL + "function newWinOpen(settings){" + NL + "\tif(!newWin){" + NL + "\t\tvar title=config.baseTitle+'->新增';" + NL + "\t\tvar url=config.inputUrl;" + NL + "\t\tnewWin=new goolov.winbox({" + NL + "\t\t\twidth:goolov.winWidth(";
  protected final String TEXT_112 = ")," + NL + "\t\t\theight:goolov.winHeight(";
  protected final String TEXT_113 = ")," + NL + "\t\t\ttitle:title," + NL + "\t\t\turl:url" + NL + "\t\t});" + NL + "\t}" + NL + "\tnewWin.open(settings);" + NL + "}" + NL + "function newWinClose(){" + NL + "\tnewWin.close();" + NL + "}" + NL + "function newData(){" + NL + "\tnewWinOpen();" + NL + "}";
  protected final String TEXT_114 = NL + "/******edit data window**********/" + NL + "var editWin;" + NL + "function editWinOpen(settings){" + NL + "\t\tif(!editWin){" + NL + "\t\teditWin=new goolov.winbox({" + NL + "\t\t\twidth:goolov.winWidth(";
  protected final String TEXT_115 = ")," + NL + "\t\t\theight:goolov.winHeight(";
  protected final String TEXT_116 = ")" + NL + "\t\t});" + NL + "\t}" + NL + "\teditWin.open(settings);" + NL + "}" + NL + "function editWinClose(){" + NL + "\teditWin.close();" + NL + "}" + NL + "function onRowDblclick(target){" + NL + "\tvar id = $(target).attr(\"id\").substr(3);" + NL + "\tvar rowIdx=$(target).attr(\"seq\");" + NL + "\tvar title=config.baseTitle+'->编辑 (序号:'+rowIdx+')';" + NL + "\tvar url=goolov.addUrlParams(config.editUrl,\"id=\"+id);" + NL + "\teditWinOpen({title:title,url:url});" + NL + "}" + NL + "function editData(){" + NL + "\tvar ids=grid.getCheckedRows();" + NL + "\tvar selCount=ids.length;" + NL + "\tif (selCount== 0) {" + NL + "\t\tgoolov.growl.info('请选择需要修改的记录,或双击需要编辑的行.');" + NL + "\t\treturn;" + NL + "\t}" + NL + "\tif(selCount>1){" + NL + "\t\tgoolov.growl.info('一次只能选择修改一项记录.');" + NL + "\t\treturn;" + NL + "\t}" + NL + "\tvar id=ids[0];" + NL + "\tedit(id);" + NL + "}" + NL + "function edit(id){" + NL + "\tif(!id){" + NL + "\t\tgoolov.growl.info('请选择需要修改的记录,或双击需要编辑的行.');" + NL + "\t\treturn;" + NL + "\t}" + NL + "\tvar rowtr=grid.getRowDom(id);//获取该ID的行元素.ID的行元素." + NL + "\tvar rowIdx=$(rowtr).attr(\"seq\");" + NL + "\tvar title=config.baseTitle+'->编辑 (序号:'+rowIdx+')';" + NL + "\tvar url=goolov.addUrlParams(config.editUrl,\"id=\"+id);" + NL + "\tvar opts={title:title,url:url};" + NL + "\teditWinOpen(opts);" + NL + "}";
  protected final String TEXT_117 = NL + "/**********add row data**************/" + NL + "function addRowData(row){" + NL + "\tvar indexId=grid.getIndexId();" + NL + "\tvar rowtr=grid.getRowDom(row[indexId]);////获取该ID的行元素." + NL + "\tif (rowtr.length==0) {" + NL + "\t\tgrid.flexAddRowData(row);\t\t" + NL + "\t}else{" + NL + "\t\tgrid.flexEditRowData(rowtr[0],row);" + NL + "\t}" + NL + "}";
  protected final String TEXT_118 = " " + NL + "/******del data**********/" + NL + "function delData(){" + NL + "\tvar ids=grid.getCheckedRows();" + NL + "\tdeleteOutJson(ids);" + NL + "}" + NL + "function del(id){" + NL + "\tif(!id){" + NL + "\t\tgoolov.growl.info('当前选择无效!');" + NL + "\t\treturn;" + NL + "\t}" + NL + "\tvar ids=new Array(id);" + NL + "\tdeleteOutJson(ids);" + NL + "}" + NL + "/******del data process**********/" + NL + "function deleteOutJson(idsArray){" + NL + "\tif (!idsArray||!$.isArray(idsArray)) {" + NL + "\t\tgoolov.growl.info('当前选择无效!');" + NL + "\t\treturn;" + NL + "\t}" + NL + "\tif (idsArray.length==0) {" + NL + "\t\tgoolov.growl.info('请选择需要删除的记录.');" + NL + "\t\treturn;" + NL + "\t}" + NL + "\tvar selCount=idsArray.length;" + NL + "\tvar idsStr=idsArray.join(',');" + NL + "\tgoolov.msgbox.confirm('删除确认','您确认删除这<span class=\"highlight\">'+selCount+'</span>项记录吗?删除后将不可恢复!',function(r){" + NL + "\t\t\tif(r){" + NL + "\t\t\t\t$.ajax({" + NL + "\t\t\t\t\ttype : 'post'," + NL + "\t\t\t\t\turl : config.deleteOutJsonUrl," + NL + "\t\t\t\t\tdata : {id:idsStr}," + NL + "\t\t\t\t\tdataType:'json'," + NL + "\t\t\t\t\tbeforeSend :function() {" + NL + "\t\t\t\t\t\tgoolov.growl.waiting('正在删除中!请稍后...');" + NL + "\t\t\t\t\t}," + NL + "\t\t\t\t\tsuccess : function(result) {" + NL + "\t\t\t\t\t\tgoolov.growl.unwaiting();" + NL + "\t\t\t\t\t\tif (result&&result.success) {" + NL + "\t\t\t\t\t\t\tgoolov.growl.success(result.message);" + NL + "\t\t\t\t\t\t\trefresh();" + NL + "\t\t\t\t\t\t}else if(result&&result.message){" + NL + "\t\t\t\t\t\t\tgoolov.msgbox.error(result.message);" + NL + "\t\t\t\t\t\t}else{" + NL + "\t\t\t\t\t\t\tgoolov.msgbox.error();" + NL + "\t\t\t\t\t\t}" + NL + "\t\t\t\t\t}," + NL + "\t\t\t\t\terror:function(request,status,errorThrown) {" + NL + "\t\t\t\t\t\tgoolov.growl.unwaiting();" + NL + "\t\t\t\t\t\tgoolov.msgbox.error();" + NL + "\t\t\t\t\t}" + NL + "\t\t\t\t});" + NL + "\t\t\t}" + NL + "\t});" + NL + "}";
  protected final String TEXT_119 = NL + "/******del all data   *******/" + NL + "function delAllData(){" + NL + "\t\tgoolov.msgbox.confirm('删除确认','您确认删除所有记录吗?删除后将不可恢复!',function(r){" + NL + "\t\tif(r){" + NL + "\t\t\t$.ajax({" + NL + "\t\t\t\t\ttype : 'get'," + NL + "\t\t\t\t\turl : config.deleteOutJsonAllUrl," + NL + "\t\t\t\t\tdataType:'json'," + NL + "\t\t\t\t\tbeforeSend :function() {" + NL + "\t\t\t\t\t\tgoolov.growl.waiting('正在删除中!请稍后...');" + NL + "\t\t\t\t\t}," + NL + "\t\t\t\t\tsuccess : function(result) {" + NL + "\t\t\t\t\t\tgoolov.growl.unwaiting();" + NL + "\t\t\t\t\t\tif (result&&result.success) {" + NL + "\t\t\t\t\t\t\tgoolov.growl.success(result.message);" + NL + "\t\t\t\t\t\t\trefresh();" + NL + "\t\t\t\t\t\t}else if(result&&result.message){" + NL + "\t\t\t\t\t\t\tgoolov.msgbox.error(result.message);" + NL + "\t\t\t\t\t\t}else{" + NL + "\t\t\t\t\t\t\tgoolov.msgbox.error();" + NL + "\t\t\t\t\t\t}" + NL + "\t\t\t\t\t}," + NL + "\t\t\t\t\terror:function(request,status,errorThrown) {" + NL + "\t\t\t\t\t\tgoolov.growl.unwaiting();" + NL + "\t\t\t\t\t\tgoolov.msgbox.error();" + NL + "\t\t\t\t\t}" + NL + "\t\t\t\t});" + NL + "\t\t}" + NL + "\t});" + NL + "}";
  protected final String TEXT_120 = NL + "/******export excel*******/" + NL + "function exportExcel(){" + NL + "\tvar paramSerialize=grid.getParamsString();" + NL + "\tvar exportExcelFileUrl=ctx+'";
  protected final String TEXT_121 = "';" + NL + "\tlocation.href=goolov.addUrlParams(exportExcelFileUrl,paramSerialize);" + NL + "}";
  protected final String TEXT_122 = NL + "/******import importExcel**********/" + NL + "var importExcelFormValidator;" + NL + "var importExcelWin;" + NL + "$(document).ready(function() {" + NL + "\timportExcelWin=$('#importExcelWin').window({" + NL + "\t\ttitle: config.baseTitle+'->导入Excel文件数据'," + NL + "\t\twidth: 450," + NL + "\t\theight: 300," + NL + "\t\tmodal: true," + NL + "\t\tshadow: true," + NL + "\t\tclosed: true," + NL + "\t\tcollapsible:false," + NL + "\t\tminimizable:false," + NL + "\t\tmaximizable:false" + NL + "\t});" + NL + "\t//表单验证" + NL + "\timportExcelFormValidator=$(\"#importExcelForm\").validate({" + NL + "\t\trules : {" + NL + "\t\t}" + NL + "\t});\t" + NL + "});" + NL + "//打开导入Excel文件窗口" + NL + "function importExcel(){" + NL + "\timportExcelWin.window('open');" + NL + "}" + NL + "//关闭导入Excel文件窗口" + NL + "function importExcelWinClose(){" + NL + "\timportExcelWin.window('close');" + NL + "\trefresh();" + NL + "}" + NL + "/**" + NL + " * 导入excel文件提交处理." + NL + " */" + NL + "function importExcelSubmit(){" + NL + "\t$(\"#importExcelForm\").ajaxSubmit({" + NL + "\t\t type : 'post'," + NL + "\t\t dataType:'html', " + NL + "\t\t url:ctx+'";
  protected final String TEXT_123 = "'," + NL + "\t\t beforeSubmit:function(formData, jqForm, options) {" + NL + "\t\t\t    if(importExcelFormValidator.numberOfInvalids()>0){" + NL + "\t\t\t\t    return false;" + NL + "\t\t\t    }" + NL + "\t\t\t    goolov.growl.waiting('导入数据中...');" + NL + "\t\t }," + NL + "\t     success:function(result, statusText, xhr, $form) {" + NL + "\t    \t \tgoolov.growl.unwaiting();" + NL + "\t\t\t\tgoolov.msgbox.info('友情提示',result?result:\"请求出现错误,请查看网络是否连接!\");" + NL + "\t\t }," + NL + "\t\t error:function(request,status,errorThrown) {" + NL + "\t\t\t goolov.growl.unwaiting();" + NL + "\t\t\t goolov.msgbox.error();" + NL + "\t\t}" + NL + "\t}); " + NL + "}";
  protected final String TEXT_124 = NL;
  protected final String TEXT_125 = NL + "/*******rel GsysCodevalue manager**********/" + NL + "var ";
  protected final String TEXT_126 = "Win;" + NL + "function ";
  protected final String TEXT_127 = "WinInit(){" + NL + "\tif(!";
  protected final String TEXT_128 = "Win){" + NL + "\t\t";
  protected final String TEXT_129 = "Win=new goolov.winbox({" + NL + "\t\t\twidth:goolov.winWidth(750)," + NL + "\t\t\theight:goolov.winHeight(450)," + NL + "\t\t\tmodal:true" + NL + "\t\t});" + NL + "\t}" + NL + "}" + NL + "function ";
  protected final String TEXT_130 = "WinOpen(settings){" + NL + "\t";
  protected final String TEXT_131 = "WinInit();" + NL + "\t";
  protected final String TEXT_132 = "Win.open(settings);" + NL + "}" + NL + "function ";
  protected final String TEXT_133 = "WinClose(){" + NL + "\t";
  protected final String TEXT_134 = "Win.close();" + NL + "}" + NL + "function ";
  protected final String TEXT_135 = "(id,name){" + NL + "\tvar title=config.baseTitle+'('+name+')->";
  protected final String TEXT_136 = "管理';" + NL + "\tvar url=ctx+'";
  protected final String TEXT_137 = "?";
  protected final String TEXT_138 = ".";
  protected final String TEXT_139 = "='+id;" + NL + "\t";
  protected final String TEXT_140 = "WinOpen({title:title,url:url});" + NL + "}";
