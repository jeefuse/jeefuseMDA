...
public class EntityManagerJSForTree
{
  protected static String nl;
  public static synchronized EntityManagerJSForTree create(String lineSeparator)
  {
    nl = lineSeparator;
    EntityManagerJSForTree result = new EntityManagerJSForTree();
    nl = null;
    return result;
  }

  public final String NL = nl == null ? (System.getProperties().getProperty("line.separator")) : nl;
  protected final String TEXT_1 = "$(function(){" + NL + "\t$('#header').height($('#header').outerHeight(true)+2);" + NL + "\t$('#container').layout({});" + NL + "\tvar w = $(\"#firstContent\").outerWidth(true);" + NL + "\tvar gridpm = 58; //GridHead，toolbar,footer,gridmargin" + NL + "\tvar h=$(\"#center\").height()-gridpm;" + NL + "    var option = {" + NL + "        height: h, " + NL + "        width: w," + NL + "        url: ctx+'";
  ...
  protected final String TEXT_37 = "\"," + NL + "\t\tautoload: true," + NL + "\t\tshowcheckbox: true," + NL + "\t\tusepager: true," + NL + "\t\tshowseq:true," + NL + "\t\trowbinddata: false," + NL + "        rowhandler: gridContextmenu," + NL + "\t\tsearchparam:searchparamsCall," + NL + "\t\tonRowDblclick:onRowDblclick" + NL + "    };" + NL + "    " + NL + "\t//format" + NL + "\t";
  ...
  protected final String TEXT_77 = "\t" + NL + "    //variate";
  protected final String TEXT_78 = NL + "    config.inputUrl=ctx+'";
  protected final String TEXT_79 = "';";
  protected final String TEXT_80 = "    " + NL + "    config.editUrl=ctx+'";
  protected final String TEXT_81 = "';";
  protected final String TEXT_82 = "    " + NL + "    config.deleteOutJsonUrl=ctx+'";
  protected final String TEXT_83 = "';";
  protected final String TEXT_84 = "   " + NL + "    config.deleteOutJsonAllUrl=ctx+'";
  protected final String TEXT_85 = "';" + NL + "     config.deleteDescendantOutJsonUrl=ctx+'";
  protected final String TEXT_86 = "';";
  protected final String TEXT_87 = NL + "    //load category tree" + NL + "    loadCategoryTree();" + NL + "});" + NL + "var grid;" + NL + "var searchFormValidator;" + NL + "var config={" + NL + "\tbaseTitle:\"";
  protected final String TEXT_88 = "管理\"," + NL + "\tinputUrl:null," + NL + "\teditUrl:null," + NL + "\tdeleteOutJsonUrl:null," + NL + "\tdeleteOutJsonAllUrl:null," + NL + "\tdeleteDescendantOutJsonUrl:null" + NL + "};" + NL + "/******grid contenxt menu process*****/" + NL + "function gridContextmenu(row) {" + NL + "\t$(row).contextmenu(menu);" + NL + "} " + NL + "/******context menu*****/" + NL + "var imgPath=ctx+\"/resources/style/default/contextmenu/images/\";" + NL + "var menu = {" + NL + "\t\twidth : 150" + NL + "\t\t,items : [" + NL + "\t\t\t\t{text : \"刷新\",icon : imgPath+\"table_refresh.png\",alias : \"refresh\",action : contextMenuClick" + NL + "\t\t\t\t,{type: \"splitLine\" }";
  protected final String TEXT_89 = NL + "\t\t\t\t,{text : \"新增\",icon :imgPath+\"view.png\",alias : \"add\",action :contextMenuClick}";
  protected final String TEXT_90 = NL + "\t\t\t\t,{text : \"编辑\",icon : imgPath+\"edit.png\",alias : \"edit\",action : contextMenuClick}";
  protected final String TEXT_91 = NL + "\t\t\t\t,{type: \"splitLine\" }" + NL + "\t\t\t\t,{text : \"删除该项记录\",icon : imgPath+\"rowdelete.png\",alias : \"deleteCurrent\",action : contextMenuClick}" + NL + "\t\t\t\t,{text : \"删除该项记录及子节点\",icon : imgPath+\"rowdelete.png\",alias : \"deleteCurrentAndDescendant\",action : contextMenuClick}" + NL + "\t\t\t\t,{text : \"删除所选记录\",icon : imgPath+\"rowdelete.png\",alias : \"deleteSels\",action : contextMenuClick}" + NL + "\t\t\t\t,{text : \"删除所选记录及子节点\",icon : imgPath+\"rowdelete.png\",alias : \"deleteDescendant\",action : contextMenuClick}";
  protected final String TEXT_92 = NL + "\t\t\t\t}]" + NL + "};" + NL + "function contextMenuClick(target,cmd) {" + NL + "\tvar domId=$(target).attr(\"id\");" + NL + "\tvar id = domId.substr(3);" + NL + "\tvar rowIdx = $(target).attr(\"seq\");" + NL + "\tswitch(cmd){";
  protected final String TEXT_93 = NL + "\t\tcase 'add':" + NL + "\t\t\tvar title=config.baseTitle+'->新增';" + NL + "\t\t\tnewWinOpen();" + NL + "\t\t\tbreak;";
  protected final String TEXT_94 = NL + "\t\tcase 'edit':" + NL + "\t\t\tvar title = config.baseTitle+'->编辑 (序号:' + rowIdx + ')';" + NL + "\t\t\tvar url=goolov.addUrlParams(config.editUrl,\"id=\"+id);" + NL + "\t\t\teditWinOpen( {title:title,url:url});" + NL + "\t\t\tbreak;";
  protected final String TEXT_95 = NL + "\t\tcase 'deleteCurrent':" + NL + "\t\t\tdeleteOutJson(new Array(id));" + NL + "\t\t\tbreak;" + NL + "\t\tcase 'deleteCurrentAndDescendant':" + NL + "\t\t\tdeleteDescendantOutJson(new Array(id));" + NL + "\t\t\tbreak;" + NL + "\t\tcase 'deleteSels':" + NL + "\t\t\tvar ids=grid.getCheckedRows();" + NL + "\t\t\tdeleteOutJson(ids);" + NL + "\t\t\tbreak;" + NL + "\t\tcase 'deleteDecendant':" + NL + "\t\t\tvar ids=grid.getCheckedRows();" + NL + "\t\t\tdeleteDescendantOutJson(ids);" + NL + "\t\t\tbreak;";
  protected final String TEXT_96 = NL + "\t\tdefault:" + NL + "\t\t\tgrid.flexReload();" + NL + "\t}" + NL + "};" + NL + "/******category tree ***************/" + NL + "function loadCategoryTree(){" + NL + "    var url=ctx+'";
  protected final String TEXT_97 = "';" + NL + "    var options = { url:url,onnodeclick:treeNodeClick,rowhandler:treeRowHandler,showcheck: false,iconspath:ctx+\"/resources/style/\"+theme+\"/tree/images/\"/*,theme: \"bbit-tree-lines\"*/ };" + NL + "    $('#firstTree').ajaxTree(options);" + NL + "}" + NL + "function treeNodeClick(item) {" + NL + "    if(!item.id||'root'==item.id){" + NL + "    \t$(\"#parentId\").val('');" + NL + "    }else{" + NL + "    \t$(\"#parentId\").val(item.id);" + NL + "    }" + NL + "\tgrid.flexReload({newp:1});" + NL + "}" + NL + "/******category tree contextmenu process************/" + NL + "var treeRowDomIdPrefix=\"firstTree_\";" + NL + "function treeRowHandler(row) {" + NL + "\tvar domId = $(row).attr(\"id\");" + NL + "\tif (!domId || domId.indexOf(treeRowDomIdPrefix) == -1)" + NL + "\t\treturn;" + NL + "\t$(row).contextmenu(menu);" + NL + "}" + NL + "function treeContextmenuClick(target,cmd) {" + NL + "\tvar domId=$(target).attr(\"id\");" + NL + "\tvar id = domId.substring(treeRowDomIdPrefix.length);" + NL + "\tvar tpath=$(target).attr(\"tpath\");" + NL + "\tvar item=$('#firstTree').getTreeViewItem(tpath);" + NL + "\tswitch(cmd){";
  protected final String TEXT_98 = NL + "\t\tcase 'add':" + NL + "\t\t\tvar title = config.baseTitle+'->新增';" + NL + "\t\t\tvar url=goolov.addUrlParams(config.inputUrl,\"parentId=\"+id);" + NL + "\t\t\tnewWinOpen({title : title,url:url});" + NL + "\t\t\tbreak;";
  protected final String TEXT_99 = NL + "\t\tcase 'edit':" + NL + "\t\t\tif (!id || id == '') {" + NL + "\t\t\t\tgoolov.growl.info('请您选择需要编辑的记录!');" + NL + "\t\t\t}" + NL + "\t\t\tvar title = config.baseTitle+'->编辑'+(item?\"(\"+item.text+\")\":\"\");" + NL + "\t\t\tvar url=goolov.addUrlParams(config.editUrl,\"id=\"+id);" + NL + "\t\t\teditWinOpen( {title:title,url:url});" + NL + "\t\t\tbreak;";
  protected final String TEXT_100 = NL + "\t\tcase 'deleteCurrent':" + NL + "\t\t\tdeleteOutJson(new Array(id));" + NL + "\t\t\tbreak;" + NL + "\t\tcase 'deleteCurrentAndDescendant':" + NL + "\t\t\tdeleteDescendantOutJson(new Array(id));";
  protected final String TEXT_101 = NL + "\t\tdefault:" + NL + "\t\t\tloadCategoryTree();" + NL + "\t}" + NL + "};" + NL + "/******context menu*****/" + NL + "var imgPath=ctx+\"/resources/style/default/contextmenu/images/\";" + NL + "var menu = {" + NL + "\t\twidth : 150" + NL + "\t\t,items : [" + NL + "\t\t\t\t{text : \"刷新\",icon : imgPath+\"table_refresh.png\",alias : \"refresh\",action : contextMenuClick" + NL + "\t\t\t\t,{type: \"splitLine\" }";
  ...
  protected final String TEXT_123 = "Win=new goolov.winbox({" + NL + "\t\t\twidth:goolov.winWidth(750)," + NL + "\t\t\theight:goolov.winHeight(450)," + NL + "\t\t\tmodal:true" + NL + "\t\t});" + NL + "\t}" + NL + "}" + NL + "function ";
  ...
  protected final String TEXT_129 = "(id,name){" + NL + "\tvar title=config.baseTitle+'('+name+')->";
  protected final String TEXT_130 = "管理';" + NL + "\tvar url=ctx+'";
  protected final String TEXT_131 = "?";
  protected final String TEXT_132 = ".";
  protected final String TEXT_133 = "='+id;" + NL + "\t";
  protected final String TEXT_134 = "WinOpen({title:title,url:url});" + NL + "}";
