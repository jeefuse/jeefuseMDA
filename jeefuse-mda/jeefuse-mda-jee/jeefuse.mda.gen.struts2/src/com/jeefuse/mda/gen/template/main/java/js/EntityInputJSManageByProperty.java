...
public class EntityInputJSManageByProperty
{
  ...
  protected final String TEXT_24 = "'," + NL + "\t\ticonspath:ctx+\"/resources/style/default/tree/images/\"," + NL + "\t\tkeyEl:'";
  ...
  protected final String TEXT_85 = NL + "    $('#inputForm').ajaxSubmit({ " + NL + "\t\t type : 'post'," + NL + "\t\t dataType:'json', ";
  protected final String TEXT_86 = NL + "\t\t data:";
  protected final String TEXT_87 = ",";
  protected final String TEXT_88 = NL + "\t\t url:submitUrl," + NL + "\t\t beforeSubmit:function(formData, jqForm, options) {" + NL + "\t\t     \tif(!$(\"#inputForm\").valid()){" + NL + "\t\t\t \t\tvar invalideNum=validator.numberOfInvalids();" + NL + "\t\t\t\t    if(invalideNum>0){" + NL + "\t\t\t\t    \tgoolov.growl.success('有<font color=red> '+invalideNum+'</font> 个选项未填写正确!');" + NL + "\t\t\t\t\t    return false;" + NL + "\t\t\t\t    }" + NL + "\t\t\t\t}" + NL + "\t\t\t\tgoolov.growl.waiting('新增数据中...');" + NL + "\t\t }," + NL + "\t     success:function(result, statusText, xhr, $form) {" + NL + "\t     \t\tgoolov.growl.unwaiting();" + NL + "\t\t\t\tif (result&&result.success) {" + NL + "\t\t\t\t\tgoolov.growl.success(result.message);" + NL + "\t\t\t\t\twindow.parent.addRowData(result.data);" + NL + "\t\t\t\t}else if(result&&result.message){" + NL + "\t\t\t\t\tgoolov.msgbox.error(result.message);" + NL + "\t\t\t\t}else{" + NL + "\t\t\t\t\tgoolov.msgbox.error();" + NL + "\t\t\t\t}" + NL + "\t\t }," + NL + "\t\t error:function(request,status,errorThrown) {" + NL + "\t\t \tgoolov.growl.unwaiting();" + NL + "\t\t\tgoolov.msgbox.error();" + NL + "\t\t}" + NL + "\t}); " + NL + "}";
  protected final String TEXT_89 = NL + "/********addNewDataWithEditWin*************/" + NL + "function addNewDataWithEditWin(){" + NL + "\tvar submitUrl=ctx+'";
  protected final String TEXT_90 = "';" + NL + "    $('#inputForm').ajaxSubmit({ " + NL + "\t\t type : 'post'," + NL + "\t\t dataType:'json', " + NL + "\t\t url:submitUrl," + NL + "\t\t beforeSubmit:function(formData, jqForm, options) {" + NL + "\t\t     \tif(!$(\"#inputForm\").valid()){" + NL + "\t\t\t \t\tvar invalideNum=validator.numberOfInvalids();" + NL + "\t\t\t\t    if(invalideNum>0){" + NL + "\t\t\t\t    \tgoolov.growl.success('有<font color=red> '+invalideNum+'</font> 个选项未填写正确!');" + NL + "\t\t\t\t\t    return false;" + NL + "\t\t\t\t    }" + NL + "\t\t\t\t}" + NL + "\t\t\t\tgoolov.growl.waiting('新增数据中...');" + NL + "\t\t }," + NL + "\t     success:function(result, statusText, xhr, $form) {" + NL + "\t     \t\tgoolov.growl.unwaiting();" + NL + "\t\t\t\tif (result&&result.success) {" + NL + "\t\t\t\t\tgoolov.growl.success(result.message);" + NL + "\t\t\t\t\t$(\"#";
  protected final String TEXT_91 = "\").val(result.data.id);" + NL + "\t\t\t\t\twindow.parent.addRowDataWithEditWin(result.data);" + NL + "\t\t\t\t}else if(result&&result.message){" + NL + "\t\t\t\t\tgoolov.msgbox.error(result.message);" + NL + "\t\t\t\t}else{" + NL + "\t\t\t\t\tgoolov.msgbox.error();" + NL + "\t\t\t\t}" + NL + "\t\t }," + NL + "\t\t error:function(request,status,errorThrown) {" + NL + "\t\t \tgoolov.growl.unwaiting();" + NL + "\t\t\tgoolov.msgbox.error();" + NL + "\t\t}" + NL + "\t}); " + NL + "}";
  protected final String TEXT_92 = NL + "/*******close edit win******/" + NL + "function editWinClose(){" + NL + "\twindow.parent.editWinClose();" + NL + "}" + NL + "function editWinCloseAndRefresh(){" + NL + "\twindow.parent.editWinCloseAndRefresh();" + NL + "}" + NL;
  protected final String TEXT_93 = NL + "var diningDefaultNoPictureUrl=ctx+'/resources/style/default/images/defaultNoPicture.jpg';" + NL + "/*******upload ";
  protected final String TEXT_94 = " image*******/" + NL + "function ";
  protected final String TEXT_95 = "Browse(e){" + NL + "\tif (!goolov.validete.image(e)) {" + NL + "\t\treturn;" + NL + "\t}" + NL + "\t$('#";
  protected final String TEXT_96 = "_image_uploaded').hide();" + NL + "\t$('#";
  protected final String TEXT_97 = "_image_browse').show();" + NL + "\tgoolov.file.previewImage(e,{previewWrapId:'";
  protected final String TEXT_98 = "_image_browse_preview',width:150,height:140});" + NL + "}" + NL + "function ";
  ...
  protected final String TEXT_102 = "_image_uploaded').show();" + NL + "}" + NL + "/*******del photo image********/" + NL + "function ";
  protected final String TEXT_103 = "Del(id){" + NL + "\tvar submitUrl=ctx+'";
  protected final String TEXT_104 = "?id='+id;" + NL + "    $.ajax({ " + NL + "\t\t type : 'post'," + NL + "\t\t dataType:'html', " + NL + "\t\t url:submitUrl," + NL + "\t\t beforeSubmit:function(formData, jqForm, options) {" + NL + "\t\t\t\tgoolov.growl.waiting('正在删除中...');" + NL + "\t\t }," + NL + "\t     success:function(result, statusText, xhr, $form) {" + NL + "\t     \t\tgoolov.growl.unwaiting();" + NL + "\t     \t\tvar result=JSON.parse(result);" + NL + "\t\t\t\tif (result&&result.success) {" + NL + "\t\t\t\t\tgoolov.growl.success(result.message);" + NL + "\t\t\t\t\t";
  protected final String TEXT_105 = "Update();" + NL + "\t\t\t\t}else if(result&&result.message){" + NL + "\t\t\t\t\tgoolov.msgbox.error(result.message);" + NL + "\t\t\t\t}else{" + NL + "\t\t\t\t\tgoolov.msgbox.error();" + NL + "\t\t\t\t}" + NL + "\t\t }," + NL + "\t\t error:function(request,status,errorThrown) {" + NL + "\t\t \tgoolov.growl.unwaiting();" + NL + "\t\t\tgoolov.msgbox.error();" + NL + "\t\t}" + NL + "\t}); " + NL + "}" + NL + "function ";
  protected final String TEXT_106 = "Update(data){" + NL + "\tgoolov.file.clear('";
  protected final String TEXT_107 = "_image');" + NL + "\t$('#";
  protected final String TEXT_108 = "_image_browse').hide();" + NL + "\tvar oldHtml=[];" + NL + "\tif(data&&data.";
  protected final String TEXT_109 = "){" + NL + "\t\toldHtml.push('<img ',' width=\"',150,'\" height=\"'+140+'\" src=\"'+ctx+'/'+data.";
  protected final String TEXT_110 = "+'\"/>');" + NL + "\t\t//oldHtml.push('<span class=\"label\">已上传</span><a href=\"javascript:";
  protected final String TEXT_111 = "Del(\\''+data.id+'\\');\" class=\"remove\" >删除</a>');" + NL + "\t\toldHtml.push('<span class=\"label\">已上传</span>');" + NL + "\t}else{" + NL + "\t\toldHtml.push('<img ',' width=\"',150,'\" height=\"'+140+'\" src=\"'+diningDefaultNoPictureUrl+'\"/>');" + NL + "\t}" + NL + "\t$('#";
  protected final String TEXT_112 = "_image_uploaded').empty().html(oldHtml.join('')).show();//.fadeOut(250, function() { $(this).remove();});" + NL + "}";
  ...
  protected final String TEXT_116 = "?id='+id;" + NL + "    $.ajax({ " + NL + "\t\t type : 'post'," + NL + "\t\t dataType:'html', " + NL + "\t\t url:submitUrl," + NL + "\t\t beforeSubmit:function(formData, jqForm, options) {" + NL + "\t\t\t\tgoolov.growl.waiting('正在删除中...');" + NL + "\t\t }," + NL + "\t     success:function(result, statusText, xhr, $form) {" + NL + "\t     \t\tgoolov.growl.unwaiting();" + NL + "\t     \t\tvar result=JSON.parse(result);" + NL + "\t\t\t\tif (result&&result.success) {" + NL + "\t\t\t\t\tgoolov.growl.success(result.message);" + NL + "\t\t\t\t\t";
  protected final String TEXT_117 = "Update();" + NL + "\t\t\t\t}else if(result&&result.message){" + NL + "\t\t\t\t\tgoolov.msgbox.error(result.message);" + NL + "\t\t\t\t}else{" + NL + "\t\t\t\t\tgoolov.msgbox.error();" + NL + "\t\t\t\t}" + NL + "\t\t }," + NL + "\t\t error:function(request,status,errorThrown) {" + NL + "\t\t \tgoolov.growl.unwaiting();" + NL + "\t\t\tgoolov.msgbox.error();" + NL + "\t\t}" + NL + "\t}); " + NL + "}";
  ...
  protected final String TEXT_123 = "Win=new goolov.winbox({" + NL + "\t\t\twidth:goolov.winWidth(750)," + NL + "\t\t\theight:goolov.winHeight(450)," + NL + "\t\t\tmodal:true," + NL + "\t\t\tneeedReLoad:false" + NL + "\t\t});" + NL + "\t}" + NL + "}" + NL + "function ";
  ...
  protected final String TEXT_138 = "WinClose();" + NL + "\tvalidator.form();" + NL + "}" + NL + "function relGlDiningSiteListForSelectClear(){" + NL + "\t$('#";
